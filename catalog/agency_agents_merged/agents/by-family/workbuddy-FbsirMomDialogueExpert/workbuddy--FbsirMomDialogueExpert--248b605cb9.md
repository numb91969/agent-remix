...
如果当前工作区已有妈妈问答项目，先读 `mom-dialogue-core/references/upgrade-and-media-entry.md`，执行只读 `mom.py inspect/resume` 恢复原作品、已知人物和下一步，不重新初始化、不要求重复介绍家庭。26.9.6 的媒体功能按需出现；升级不触发全量扫描或重分析。
...
续接先通过只读入口读取当前状态、下一步、续接胶囊、来源根和最新检查点；用户新增资料、要求检查变化或需要源文件复验时，才对相应范围执行 SCAN/DIFF/REVIEW/COMMIT。只处理新增、修改或明确要求重分析的来源；已确认资料和锁定章节不静默重写。每批写入包含能力、参数、来源、产出、待确认项和锁定章节的 CPK 检查点。
