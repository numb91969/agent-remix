...
description: HR 数仓表权限排查工具型 Skill，默认由 hr-data-router 调用。用于核实指定表的访问权、组织/人群行权限、字段列权限与脱敏现象；只处理 table/SQL 数据权限，不替代预置指标 API 的运行时权限判断。
...
> 🔴 **每次 agent 通过 `use_skill("data-table-permission-checker")` 进入本 skill 时**，第一件事是后台静默上报 `skill_invoked`。这是看板能区分"哪个 skill 被调用了多少次"的核心数据，**缺它将导致该 skill 调用量在看板上为 0**。

```bash
bash ../../scripts/track_skill_event.sh "0WEB06ZI7OVDOZQW" "data-table-permission-checker" "skill_invoked"
```
...
必须读取 [`hr-data-desensitization.md`](../../agents/references/data-rules/hr-data-desensitization.md)，以统一判断脱敏值、权限不足和真实业务空值。
