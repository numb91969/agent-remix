...
| 歧义场景 | 判定规则 |
|----------|---------|
| `us-*` / `wedata-*` 开头的任务 ID 该走调度平台还是运行时诊断 | **严格看四段式**：命中 `^(us\|wedata)-\d+-\d+-\d+$`（前缀 + 恰好 3 段纯数字、共 4 段，如 `wedata-2026070103102338-1782848143426-3`）→ 是**峰峦运行时 job_id**，走 `yarn-app-diagnose`（`engine=fengluan`）；**不满足四段式**（尤其**纯数字**：US 18 位 / WeData 17 位）才是上层调度任务 ID，走 `us-fail-task-diagnose` / `wedata-instance-ops`。[WARN] 用峰峦 job_id 去 US/WeData 按任务 ID 查一定查不到 |
| `livy-sessionid-*` 开头的 ID 该走 yarn-app-diagnose 还是 SuperSQL | **优先 yarn-app-diagnose**：`livy-sessionid-*` 是 SuperSQL 经 Livy 提交的 Spark 作业（峰峦 platform=`US-SUPERSQL`），归属峰峦运行时 job_id，直接走 `yarn-app-diagnose`（`engine=fengluan`，app-info 自动识别）做失败/日志诊断；**仅当进一步需要 SuperSQL 全链路上下文**（SQL、错误码、Failover、引擎参数）时，再转 `SuperSQL/`（supersql-job-analyzer / supersql-slow-query-analyzer） |
