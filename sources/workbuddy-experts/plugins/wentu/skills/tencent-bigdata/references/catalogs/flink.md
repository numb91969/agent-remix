...
- **核心能力**:
  ...
  - **支持的 Oceanus 环境**: `pub_oceanus2.0` / `pcg_new_oceanus2.0` / `pcg_oceanus2.0` / `fit_oceanus2.0` / `pre_oceanus2.0` / `pub_oceanus1.0` / `sg_oceanus2.0` / `sg_oceanus1.0` / `wxgpay_oceanus2.0` / `sg_public_qcloud_oceanus2.0`（海外公有云 wedata sg_public），共 10 个；URL 自动识别；海外公有云需附加 `--skill-namespace overseas-sg`
