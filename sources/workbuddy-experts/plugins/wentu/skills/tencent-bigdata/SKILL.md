...
| Yarn | `sub-skills/Yarn/` | YARN、Application、app_id、`application_xxx`、`job_xxx`、队列资源、queue、Container killed、Executor lost、OOM、Spark失败、MapReduce失败、AM日志、知识库检索、应用组可用集群、appgroup 集群、gaia_id 查询、**峰峦、Fengluan、K8s on Spark、`us-`/`wedata-` 严格四段式峰峦 job_id、`livy-sessionid-*` 峰峦 job_id** | YARN / 峰峦（K8s on Spark）**运行时任务**失败诊断（含知识库检索辅助）、队列资源分析、应用组可用计算集群查询 |
...
## 运行时任务 ID 形态识别（强制 — 先判定 ID 类型，再交既定规则路由）

> [WARN] **`us-*` / `wedata-*` 既可能是「平台任务 ID」，也可能是「峰峦运行时 ID」，两者性质完全不同。仅凭 `us` / `wedata` 关键字臆测会判错类型。** 命中 US / WeData 相关关键字时，**必须先判定 ID 类型**，把"这是哪类 ID"的事实补充清楚，再交给下游既定规则路由。
>
> [WARN] **本层的职责边界（务必牢记）**：本层**只做一件事——识别并补充「这个 ID 是什么类型」的事实信息**（峰峦运行时 ID / Yarn 运行时 ID / US 平台 ID / WeData 平台 ID）。**本层不决定子系统、不决定子 skill 入口、不给出任何默认候选**；ID 类型只是下游路由规则的**一个输入**，具体「走哪个子系统、选哪个子 skill」完全由既有的「子 skill 执行路径约束」（用户意图 + 子系统 `SKILL.md` + `catalogs/*.md`）按既定规则裁定。

| ID 形态 | 示例 | 含义 | ID 类型（本层仅补充此事实） |
|---------|------|------|---------|
| **严格四段式**：`us-<数字>-<数字>-<数字>` / `wedata-<数字>-<数字>-<数字>`（即 `^(us\|wedata)-\d+-\d+-\d+$`，前缀 + 恰好 3 段纯数字，共 4 段） | `wedata-2026070103102338-1782848143426-3`、`us-2026070103102338-1782848143426-3` | 峰峦（K8s on Spark）提交的作业 | **峰峦运行时 ID**（注意：不是 US/WeData 平台任务 ID） |
| `livy-sessionid-*` 前缀 | `livy-sessionid-31118986-…` | SuperSQL 经 Livy 提交的 Spark 作业（platform 恒为 `US-SUPERSQL`） | **峰峦运行时 ID** |
| 纯数字 18 位 | `100xxxxxxxxxxxxxxx` | US 统一调度产生的任务 | **US 平台 ID**（上层调度任务，非运行时作业） |
| 纯数字 17 位 | `20xxxxxxxxxxxxxxx` | WeData 纯调度产生的任务 | **WeData 平台 ID**（上层调度任务，非运行时作业） |
| `application_xxx` / `job_xxx` | `application_1757659972062_13336918` | YARN 上的应用 | **Yarn 运行时 ID** |

> [OK] **判定要点（本层只负责把"类型"判对）**：
>
> 1. **`us-*` / `wedata-*` 必须严格命中四段式** `^(us|wedata)-\d+-\d+-\d+$`（前缀后跟恰好 3 段纯数字、共 4 段）才判定为**峰峦运行时 ID**；**不满足四段式**（尤其是**纯数字**：US 18 位 / WeData 17 位）的，判定为 **US / WeData 平台 ID**。[WARN] 用峰峦运行时 ID 去 US/WeData 按任务 ID 查一定查不到——若下游查不到，多半是类型判错了，回头重判。
> 2. **`livy-sessionid-*` 类型为峰峦运行时 ID**（SuperSQL 经 Livy 提交的 Spark，platform 恒为 `US-SUPERSQL`）。
> 3. **平台 ID（纯数字）的"子 skill 由下游按意图选"，本层不管。** 拿到 US/WeData 平台 ID 后，用户意图（失败诊断 / 慢诊断 / 超时 / 实例运维 / 任务操作 / 日志查询…）不同，应选的子 skill 与归属子系统也不同（如 WeData 任务的慢诊断可能落到 `US/`），**切勿因 ID 是 `wedata*` 前缀就直接选 WeData 下"最像"的子 skill**——具体选哪个、归哪个子系统，交给「子 skill 执行路径约束」（意图 + 子系统 `SKILL.md` + `catalogs/*.md`）裁定。
