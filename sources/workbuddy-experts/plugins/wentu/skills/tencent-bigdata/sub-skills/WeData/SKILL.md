...
> [WARN] **边界：`wedata-<数字>-<数字>-<数字>` 严格四段式的 ID 不是 WeData 任务 ID，不要在本子系统处理。**
>
> WeData **纯调度任务 ID 是纯数字（17 位）**。如果用户给的 ID **严格命中四段式** `^wedata-\d+-\d+-\d+$`（`wedata-` 前缀 + 恰好 3 段纯数字，如 `wedata-2026070103102338-1782848143426-3`），那是**峰峦（K8s on Spark）运行时 job_id**，不是 WeData 调度任务——用它去 WeData 按任务 ID 查一定查不到。此时**必须改用 `Yarn/yarn-app-diagnose`**（app-info 会自动识别为 `engine=fengluan`）。同理 `us-` 四段式也是峰峦 job_id；`application_xxx` / `job_xxx` 亦走 `Yarn/yarn-app-diagnose`。
>
...
### 15. wedata-plugsql — 创建 WeData PlugSQL 任务（端到端）

- **路径**: `wedata-plugsql/`
- **用途**: 在 WeData 上**从零创建并发布一个 PlugSQL 离线任务**（task_type=3）的端到端编排
- **触发场景**:
  - 用户明确要创建 / 发布一个 **PlugSQL（插件 SQL，task_type=3）任务**
  - 用户需要走"上传脚本 → 建任务 → 配目标表 → 配 git → 发布前校验 → 发布"完整链路
- **不触发场景**:
  - 其它类型离线任务（SuperSQL / PySpark / Shell 等）的通用增删改查 → `wedata-task-dev`
  - 任务实例运维（日志 / 重跑 / kill） → `wedata-instance-ops`
  - 即席 SQL → `wedata-sql-explore`
- **核心能力**:
  - 通过 `do-bigdata wedata plugsql` 二级分组的新增命令（`describe-file` / `platform-accounts` / `boundary-list` / `authorized-db` / `save-target-table` / `validate` / `transfer-main-file`）+ 复用 `upload file` / `task-dev create|modify|publish` 串联完整流程
  - 目标表配置（子域 / 授权库 / 目标表草稿）、发布前代码校验、代码转换并存储
- **触发关键词**: plugsql、PlugSQL、插件 sql、创建 plugsql 任务、目标表草稿、配置目标表、saveEntityDraft、发布前校验、代码转换存储、wedata 创建插件 sql 任务
<!-- /skill:wedata-plugsql -->

<!-- skill:dataquality -->
### 16. dataquality — 数据质量监控规则创建/查询/删除

- **路径**: `dataquality/`
- **用途**: 创建 WeData 数据质量监控规则，支持三种交互模式（推荐/引导/执行）。对表做一致性对比监控、配置定时检测和告警。也支持查询已有规则、删除规则。
- **触发场景**:
  - 用户想要对表创建数据质量监控规则
  - 用户想要对两张表做一致性对比监控
  - 用户想要配置定时检测和告警（企微/邮件/电话/群聊机器人）
  - 用户想要查看或删除已有的质量规则
- **不触发场景**:
  - 查数据/跑 SQL → `wedata-sql-explore` / `sql-execute-analyze`
  - 查表结构/血缘 → `wedata-metadata` / `datamap`
  - 任务实例运维 → `wedata-instance-ops`
- **核心能力**:
  - 通过 `do-bigdata wedata dataquality` 二级分组 CLI 命令调用（7 个原子命令）
  - 支持单表规则（行数/空值/重复/聚合/枚举/自定义SQL）+ 两表一致性对比
  - 三种交互模式自动切换
  - 提交前自检（防 DqUnknownException）
  - CMK 凭证由 `@auth_required` 中间件自动加载（加密存储，三级 fallback）
- **触发关键词**: 数据质量、质量监控、质量规则、空值率、重复率、一致性监控、告警阈值、规则组、dataquality、DQ、创建监控、删除监控、规则草稿、字段枚举值、字段聚合
<!-- /skill:dataquality -->
...
<!-- skill:wedata-plugsql -->
| 创建 / 发布一个 PlugSQL（插件 SQL，task_type=3）任务 | wedata-plugsql |
| 走"上传脚本→建任务→配目标表→配 git→校验→发布"完整链路 | wedata-plugsql |
| 配置 plugsql 目标表 / 目标表草稿 / saveEntityDraft | wedata-plugsql |
<!-- /skill:wedata-plugsql -->
<!-- skill:dataquality -->
| 创建数据质量监控规则 / 对表做质量监控 | dataquality |
| 两表一致性对比监控 | dataquality |
| 配置定时检测和告警（企微/邮件/电话/群聊） | dataquality |
| 查看或删除已有的质量规则 | dataquality |
<!-- /skill:dataquality -->

| 提供 `wedata-<数字>-<数字>-<数字>` / `us-<数字>-<数字>-<数字>` 严格四段式 ID 诊断失败 | **不属于 WeData**：这是**峰峦运行时 job_id**，转 `Yarn/yarn-app-diagnose`（WeData 调度任务 ID 是纯数字 17 位，命中 `^(us\|wedata)-\d+-\d+-\d+$` 四段式的不是） |
| 提供 `livy-sessionid-*` 形态 ID 诊断失败 | **不属于 WeData**：这是 SuperSQL 经 Livy 提交的 Spark 作业（峰峦运行时 job_id），**优先转 `Yarn/yarn-app-diagnose`**（`engine=fengluan`），仅当需要 SuperSQL 全链路上下文时才转 `SuperSQL/` |
...
## 子 Agent 转诊单输出规则（多 Agent 架构）

> 仅在 WeData 作为独立子 Agent 运行时适用。**不新增诊断逻辑**，只把本 SKILL.md 已有的跨系统路由改为结构化「转诊单」，交主 Agent 扁平转发。输出固定 JSON 字段：`status`、`next_agent`、`handoff_payload`、`new_findings`；完整通用契约由子 Agent Prompt 注入，不要求本子树读取根 `references/`。

- 数据探索任务日志链接 / WeData TaskId（对应上文 explore-log-analyzer）：先用 `do-bigdata wedata explore-query-session` 换取 SuperSQL SessionId(UUID)，再 `status=need_handoff`、`next_agent=SuperSQL链路诊断`、`handoff_payload.ids.session_id=<SessionId>`。
- 实例引擎层失败 / 需 Yarn/Spark UI 深度排障（对应上文 wedata-instance-ops 的引擎日志链接） → `status=need_handoff`、`next_agent=YarnSpark计算作业诊断`、`handoff_payload.ids.application_id=<app_id>`。
- 其余执行/开发/查询/SQL 失败诊断（supersql-diagnosis 等本域能力）完成 → 输出 `status=done`。
