...
| **资源分配不足** | App 启动到第一个 Task 开始间隔大；实际 Executor 数远小于 maxExecutors | ① 检查队列资源是否充足（联动 `yarn-queue-analysis`）<br>② 调整 `spark.dynamicAllocation.minExecutors` 保持基础资源<br>③ 错峰提交或申请更多队列配额<br>④ **确认队列资源确实不足（`yarn_queue_context.contention_level` 为 `high`/`severe`，或 `analyzer_resource_wait.py` 产出 `scene=resource_wait`）时，必须按下方「[资源不足 → 扩容引导]」小节输出可操作的扩容引导（联系人 + 申请页面 + 目标集群）** |
...
> # 从 executorLogs 字段提取 container URL，然后用 log-content 搜索
> do-bigdata yarn log-content --log-url {spark.log_url} --start 2097152 --grep "spilling sort data|Failed to allocate a page|ExecutorOutOfMemory|Creating normal reader" --out-file auto
> ```
>
...
>
> ### 场景 D：极低并行度 → SQL plan 自动补位（一步到位）
>
> **触发条件**（满足以下**全部**）：
> 1. `findings[scene=stage_slow].detail.affected_stages[]` 中存在 Stage 满足 `dispatched_tasks ≤ 2` 且 `duration_sec > 600`（10min）
> 2. 该 Stage 的 `branch_data` 中**不存在** `low_parallelism` 字段（或为 null），即脚本未自动检测到极低并行度
> 3. 顶层 `sql_plan_insights` 为 null / 空，或该 Stage 无 `sql_plan_insight` 字段
>
> **背景**：`_detect_low_parallelism` 是后增逻辑，老版本脚本或某些边界条件（如 wave_count=None / task metrics 未上报）下可能跳过该检测，导致 S7 SQL plan 触发条件不满足。此时 AI 必须**自动补位**，不等用户追问。
>
> **AI 必须自动执行的步骤**（诊断输出后立即执行，无需用户确认）：
>
> ```bash
> # Step 1: 获取 SQL 执行计划（拿到物理 plan + accumulatorUpdates）
> do-bigdata spark sqls --app-id <app_id> --details -o json
> ```
>
> ```bash
> # Step 2: 调用 SQL plan 分析脚本，针对极低并行度的慢 Stage 进行 plan 层诊断
> cd <spark-slow-analyzer/scripts 目录>
> python3 analyzer_sql_plan.py --app-id <app_id> --stage-ids <sid1>,<sid2>,... --output /tmp/spark_sql_plan_result.json
> ```
>
> 若 `analyzer_sql_plan.py` 脚本不可用或执行失败，手动补位：
> 1. 从 `sqls --details` 输出中提取物理 plan（`physicalPlanDescription` 字段）
> 2. 通过 `accumulatorUpdates` 中的 `stageId` 字段，将慢 Stage 映射到 plan 中的具体算子节点
> 3. 分析该 Stage 对应节点的上下游算子链路，重点关注：
>    - **Join 类型**（SortMergeJoin / BroadcastHashJoin / CartesianProduct）及其 join key
>    - **数据膨胀**：比较 Stage 的 `shuffle_read_bytes`（输入）vs `shuffle_write_bytes`（输出），计算膨胀倍数
>    - **Expand 算子**：是否存在 `Expand` 节点（通常由 COUNT(DISTINCT) 或 GROUPING SETS 产生，会使数据行数翻倍）
>    - **AQE 分区合并**：是否存在 `AQEShuffleRead(coalesced)` 把多个分区合并为 1 个
>    - **全局聚合**：`HashAggregate` 无 grouping keys → 必然产生单分区
>
> **判定逻辑**（AI 基于 plan 结果给出精准结论）：
>
> | Plan 特征 | 根因判定 | 建议 |
> |-----------|---------|------|
> | SortMergeJoin + 膨胀倍数 > 100x | **多对多 Join 数据爆炸**：join key 存在大量重复，导致笛卡尔积式膨胀 | ① 对 Join 的一侧预聚合/去重（`SELECT DISTINCT join_key FROM ...`）；② 检查 JOIN ON 条件是否遗漏了限定字段 |
> | Expand + COUNT(DISTINCT) | **Expand 算子使数据翻倍**：每个 COUNT(DISTINCT) 会增加一倍数据行 | ① 两阶段聚合：先 GROUP BY 细粒度去重，再聚合；② 如果多个 COUNT(DISTINCT)，考虑拆分为多个子查询 UNION |
> | AQEShuffleRead(coalesced) + 计算密集 | **AQE 过度合并分区** | `SET spark.sql.adaptive.coalescePartitions.enabled = false;` |
> | HashAggregate 无 grouping keys | **全局聚合必然单分区** | 改写 SQL 增加 GROUP BY 维度，或拆分为多步 |
> | Repartition(1) / Coalesce(1) | **代码/框架强制单分区** | 删除 coalesce(1) 或改为合理值 |
>
> **输出要求**：在诊断报告中自动附加一个「SQL Plan 深度分析（AI 自动补位）」小节，格式如：
> ```
> ═══════════════════════════════════════════════════════════
> [LIST] SQL Plan 深度分析（AI 自动补位 · 极低并行度场景）
> ═══════════════════════════════════════════════════════════
>
> 触发原因：Stage 33/28/31 dispatched_tasks=1，耗时数小时，但脚本未自动执行 SQL plan 分析
>
> Stage 33 (耗时 4.3h, dispatched_tasks=1):
>   Plan 链路：Exchange → Sort → SortMergeJoin(LeftOuter, [key1=key2]) → Expand → HashAggregate
>   数据膨胀：shuffle_read=2.5MB → shuffle_write=46.7GB（膨胀 17,709x）
>   根因：SortMergeJoin LeftOuter 产生多对多匹配（右表 dau_di_v2 单 key 有大量重复行），
>        + Expand 算子再翻倍（COUNT(DISTINCT) 展开）
>   建议：
>     P0: 对右表预去重 → SELECT DISTINCT user_id, ds FROM dau_di_v2 WHERE ...
>     P1: 两阶段聚合消除 Expand → 先 GROUP BY 全部维度 + DISTINCT 列去重，再外层聚合
> ```
>
> **关键原则**：
> - 这一步是**自动执行**的，不需要在报告末尾写"建议进一步分析 SQL plan"之类的引导语
> - 结论必须基于 plan 实际数据，不能凭空推测
> - 如果 plan 拉取失败，明确告知"SQL plan 获取失败，原因：XXX"，不编造
...
> - **[FAIL] BroadcastNestedLoopJoin 不等于笛卡尔积瓶颈 — 必须检查 output_rows**：SQL Plan 中出现 `BroadcastNestedLoopJoin`（BNLJ）**不代表存在性能问题**。BNLJ 有两种截然不同的场景：
>   - **真笛卡尔积**：大表 × 大表 / 大表 × 小表无等值条件 → output_rows 巨大（百万~亿级），O(N×M) 性能杀手
>   - **标量子查询拼接**：`SELECT (SELECT COUNT(..) FROM A), (SELECT SUM(..) FROM B)` 等场景，Spark 用 BNLJ(condition=true) 把多个聚合结果（每个 1 行）拼接 → output_rows=1，开销为零
>
>   **判定规则**：看 BNLJ 节点的 `number of output rows` metric：
>   - output_rows ≤ 1000 → **标量拼接**，severity=info，不列为根因/瓶颈，不推荐改写 SQL
>   - output_rows > 1000 或无法确认 → **真笛卡尔积**，severity=high，列为根因并推荐改写
>
>   **严禁**：仅凭 Plan 中存在 BNLJ 节点就断言"SQL 中存在笛卡尔积"并推荐"改写 SQL 增加 JOIN 条件"。必须先检查 output_rows 确认是否真的产生了数据膨胀。脚本已自动处理此逻辑（`is_scalar_join` 字段），AI 在输出诊断结果时若 `warnings[].is_scalar_join=true`，不得将其列为根因。
>
...
>
> [PIN] **极低并行度（low_parallelism）场景的深度诊断**：
>
> **触发条件**：Stage 的 `dispatched_tasks` ≤ 2 且 `duration` > 10min
>
> **背景**：当一个 Stage 只有 1~2 个 task 却跑了几十分钟甚至数小时时，问题核心不是
> "Task 计算效率低"，而是"为什么只有这么少的 task，无法利用集群并行能力"。
> 这类场景在原有 wave_count 分支中会被归入"分支 A：Task 本身慢"，只输出
> dominant_cause=exec_run，但不提示"问题出在并行度太低"。
>
> **脚本已自动执行**：`analyzer_stage_slow.py` 的 `_detect_low_parallelism` 会在
> wave_count ≤ 2 分支中自动检测，结果存放在：
> - `affected_stages[].branch_data.low_parallelism` — 检测结果
> - `branches` 数组中包含 `"low_parallelism"` — 标识命中
> - 自动触发 S7 SQL plan 分析（无视 500GB 阈值）
>
> **AI 必须执行的下一步**：
> 1. 查看 `branch_data.low_parallelism.cause_category` 确认根因分类：
>    - `shuffle_low_partition`：shuffle 读端分区极少 → 查 AQE 合并 / 全局聚合 / coalesce
>    - `scan_single_split`：scan 端文件被合并为 1 个 split → 查 CombineHiveInputFormat
>    - `no_data_source_detected`：无 I/O 数据 → 需看 plan 确认
>
> 2. 消费 SQL plan 结果（S7 触发后自动回填到 `sql_plan_insight`），确认：
>    - **AQEShuffleRead + CustomShuffleReader(coalesced)** → AQE 合并了分区过头
>    - **CartesianProduct / BroadcastNestedLoopJoin** → 笛卡尔积（少量数据但 O(N²) 计算）；**注意**：必须检查 output_rows，若 ≤ 1000 则是标量子查询拼接（非瓶颈），参见上方 PIN 规则
>    - **GlobalLimit / Aggregate(无 grouping keys)** → 全局聚合必然单分区
>    - **Repartition(1) / Coalesce(1)** → 用户代码/ETL 框架强制
>
> 3. 给出精准建议（按 cause_category）：
>
> | 原因分类 | 判定依据 | 建议 |
> |---------|---------|------|
> | **AQE 过度合并** | shuffle_read > 0, task=1, 计算密集 | `SET spark.sql.adaptive.coalescePartitions.enabled = false;`<br>后续观察：若关闭后仍慢，基本可断定是数据倾斜，需排查倾斜 key 做针对性优化 |
> | **全局聚合 / scalar subquery** | plan 中有 Aggregate 无 grouping keys / ScalarSubquery | 改写 SQL：增加 GROUP BY 维度 或 拆分为多步 |
> | **笛卡尔积/BNLJ** | plan 中有 CartesianProduct / BroadcastNestedLoopJoin **且 output_rows > 1000**（排除标量拼接） | 改写 SQL，增加 JOIN 条件 |
> | **explode + 复杂计算** | input 小但 exec_run 占比 > 95% | 优化 UDF / repartition 后再处理 |
> | **上游 coalesce(1)** | plan 或代码中存在 coalesce/repartition(1) | 删除 coalesce(1) 或改为合理值 |
> | **Scan 单 split** | input 小 + CombineHiveInputFormat 合并 | `SET spark.sql.files.maxPartitionBytes = 1048576;`（Spark 3.x）<br>或 `SET mapred.max.split.size = 1048576;`（Spark 2.x） |
>
> **输出要求**：在诊断报告中必须包含一个「极低并行度根因」小节，格式如：
> ```
> 极低并行度检测：Stage 33 (dispatched_tasks=1, 耗时=4h18min)
> 根因分类：shuffle_low_partition
> Plan 确认：AQEShuffleRead(coalesced partitions: 100→1) + HashAggregate(无 GROUP BY)
> 结论：AQE 把 100 个 shuffle 分区合并为 1 个（因 shuffle_read 仅 2.5MB），
>       但后续 explode + 复杂 UDF 导致计算量远超预期，应禁止合并
> 建议：SET spark.sql.adaptive.coalescePartitions.enabled = false;
>       后续观察：若关闭后仍慢，基本可断定是数据倾斜，需排查倾斜 key 做针对性优化
> ```
>
> **兜底（脚本未检测到 low_parallelism 时的 AI 自动补位）**：
>
> 当 AI 在 `diagnose` 输出中观察到 Stage 满足 `dispatched_tasks ≤ 2` + `duration_sec > 600`，但 `branch_data` 中**没有** `low_parallelism` 字段（即脚本版本不支持或检测逻辑未覆盖），AI 必须**立即执行上方「场景 D」的自动补位流程**，不等用户追问。
>
> 典型触发信号（diagnose JSON 中可直接观察）：
> - `affected_stages[].dispatched_tasks = 1` 且 `duration_sec > 3600`（1小时+）
> - `branches` 为 `['task_intrinsic']` 或 `['unknown']`（没有 `low_parallelism`）
> - `sql_plan_insights` 为 null / 不存在
>
> **这条规则的优先级高于"输出诊断报告后结束"**——即使脚本的诊断报告看似完整（有 verdict、有 branches），只要满足上述条件，AI 仍必须自动追加 SQL plan 分析。
...
**Step 2：获取 Driver 日志文件列表 → 首轮聚合扫描（首选）→ 按线索多轮 grep 深挖 → 分析**

```bash
# 获取日志文件列表
do-bigdata yarn log-list --container-url {am_container_logs} --query "<用户原始问题>"

# 【首选首轮】聚合扫描：一次并行扫完整个容器全部 size>1 文件的整套错误关键字（含 Aborting job / Job aborted due to stage failure）
# 命中多时 JSON 较大，建议默认加 --out-file auto 落盘，避免 agent stdout 截断，再用 read_file 无截断读取
do-bigdata yarn log-scan --container-url {am_container_logs} --profile spark --out-file auto --query "<用户原始问题>"

# 快速查看日志尾部（首轮扫描完全无命中时才用）
do-bigdata yarn log-content --log-url {stderr_url} --start 8192 --query "<用户原始问题>"

# 单文件深挖：一次正则多关键字（放大 --start 到 40MB 覆盖全量；带 --grep 全量深挖时同样建议 --out-file auto）
do-bigdata yarn log-content --log-url {stderr_url} --start 40960000 --grep {keyword} --context 3 --out-file auto --query "<用户原始问题>"

# 增大读取字节量以覆盖更早的日志（--start 是"从文件末尾读取的字节数"，传正/负数一律按末尾计，无"从头部读取"语义）
do-bigdata yarn log-content --log-url {log_url} --start {bytes} --query "<用户原始问题>"
```

> [ROCKET] **首选做法**：先用 `log-scan --profile spark` 一次聚合扫完整个容器，再根据命中线索对**命中文件**追加 `log-content --grep` 深挖，**不要**跳过首轮扫描直接读日志尾部（很多关键错误如 SyntaxError/ImportError 出现在日志中间而非末尾）。**仅当首轮扫描完全无命中时**，才回退到 `log-content --start 8192` 读尾部。
>
> * **大输出落盘（全局强制要求）**：`log-scan` / `log-content`（尤其 `--grep` 全量深挖）等命令的完整 JSON 体积大，**部分 agent 框架会在固定字节处（如 51200 bytes）硬截断命令行 stdout**。对这类命令**一律加 `--out-file auto`**——完整 JSON 落到 `~/.do-bigdata/output/`，且落盘文件的**绝对路径会前置打印在 stdout 开头**（即使后续摘要被框架截断，也能先拿到路径）。
> **一旦发现命令输出被截断（stdout 末尾出现 `truncated` / 内容明显不完整 / JSON 无法解析），不要基于残缺输出下结论、也不要重复执行命令，而是立刻用 `read_file` 读取 stdout 开头给出的落盘文件绝对路径**（可配合 `offset` / `limit` 分页）拿到无截断的全量结果。CLI 会自动回收历史文件，**不要**用 `rm` 手删。

**日志文件优先级**：`stderr` > `spark.log` > `stdout` > `gc.log`

> [WARN] **关键约束**：Driver 日志可能数十 MB，无法一次性全部读取。首轮用 `log-scan` 聚合扫描覆盖全量，再对命中文件用 `grep` 分多轮定向深挖，综合分析。
...
> [PIN] **注意**：此流程仅借用 `yarn-app-diagnose` 的日志获取能力（通过 `do-bigdata yarn log-list` / `log-scan` / `log-content` CLI 命令，大输出配 `--out-file auto`），不使用 `yarn-app-diagnose` 的诊断分析逻辑。
...
| `yarn-app-diagnose` | ① Application 失败时 → 联动 Yarn 失败诊断分析日志；② **History Server 指标全部获取失败时** → 仅借用其日志获取能力（通过 `do-bigdata yarn app-info`、`do-bigdata yarn log-list`、`do-bigdata yarn log-scan`、`do-bigdata yarn log-content` 命令，大输出配 `--out-file auto`）拉取 Driver/Executor 日志，分析在本 Skill 内完成 |
...
当诊断命中**资源分配不足**（诊断维度表「资源分配不足」行，或 `analyzer_resource_wait.py` 产出 `scene=resource_wait`，或 `yarn_queue_context.contention_level` 为 `high`/`severe`）时，AI **必须**在诊断报告中额外输出一段可操作的中文扩容引导，明确「联系人（应用组负责人）+ 申请页面 + 目标集群」，而不仅是「申请更多配额」这类方向性建议。

**引导话术（固定格式）**：

```
如需扩容，请联系应用组 <appgroup> 负责人 <owners>，在 https://wedata.woa.com/appGroup 页面为 <集群中文名> 集群申请扩容
```

**第 1 步：获取应用组与集群（按优先级，命中即用，不必逐个尝试）**

1. **优先复用诊断上下文**：若 `diagnose`/`compare` 输出的 `yarn_queue_context` 已含 `appgroup` 与 `cluster`，直接使用，无需再调命令。
2. **峰峦任务（`app_info.engine=fengluan`）**：调用 `do-bigdata yarn app-info --app-id <app_id>`，从返回的 `app_group` 字段取应用组，从 `cluster_info` 字段取集群中文名（`cluster_info.ch_name`）。
3. **YARN 任务（`app-info` 结果无 `app_group` 字段）**：调用 `do-bigdata yarn appgroup-info --app-id <app_id>`，从返回取应用组（`appgroup_name`）与集群（`cluster_name` / `cluster_display_name`，优先展示中文名）。

```bash
# 峰峦任务：从 app-info 取应用组与集群中文名
do-bigdata yarn app-info --app-id <app_id> -o json      # 读取 app_group、cluster_info.ch_name

# YARN 任务：回退到 appgroup-info 反查应用组与集群
do-bigdata yarn appgroup-info --app-id <app_id> -o json  # 读取 appgroup_name、cluster_display_name
```

**第 2 步：根据应用组查负责人**

```bash
do-bigdata yarn appgroup-owners --appgroup <appgroup> -o json   # 读取 owners 列表（可能多人）
```

- 负责人可能有多个，话术中一并列出（如 `zhangsan、lisi`）。

**第 3 步：按固定话术输出引导**，将 `<appgroup>` / `<owners>` / `<集群中文名>` 替换为实际值。例如：

```
如需扩容，请联系应用组 g_teg_competition_superteam3 负责人 zhangsan、lisi，在 https://wedata.woa.com/appGroup 页面为 TDW南京集群 集群申请扩容
```

**降级处理（不阻断诊断主流程）**：

- **未查询到负责人**（`owners` 为空或查询失败）：负责人处降级提示，话术改为「…请联系应用组 `<appgroup>` 负责人（未查询到负责人，请在应用组管理页确认），在 https://wedata.woa.com/appGroup 页面…」，页面链接与目标集群照常输出。
- **集群中文名缺失**（映射表未命中）：回退使用集群英文名/别名，话术仍可读。
- 上述任何一步失败都**不得中断**主诊断流程，扩容引导作为附加建议输出即可。
