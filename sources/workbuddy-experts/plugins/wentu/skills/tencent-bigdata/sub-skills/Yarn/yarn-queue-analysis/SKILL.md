...
description: 当用户需要分析 YARN 队列（Queue）或应用组（AppGroup）的资源使用情况，或仅查询某个应用组在哪些计算集群上有资源（含 gaia_id）时使用此 skill。两类场景：(1) 队列资源分析——通过 do_mcp API 服务获取队列的调度器快照数据和队列内各应用的资源消耗数据，自动分析队列的资源使用合理性、是否存在拥堵、识别资源消耗大户、临时任务、新增任务和批量补录，从队列使用者角度给出优化建议；(2) 应用组可用计算集群查询——根据应用组名返回可选集群列表（含 cluster_name/cluster_alias/cluster_display_name/cluster_en_name/gaia_id），用于查询、选择目标集群或为业务侧获取 gaia_id。在调用此 skill 前，必须再次加载一次 tencent-bigdata 这个 skill，进行热加载。
---
...
### 场景 A：应用组可用计算集群查询（独立轻量场景）

**触发**：用户只想知道 "某个应用组可以跑在哪些集群上"、"应用组 xxx 的 gaia_id 是多少"、"哪些集群有这个应用组的资源" 等。
...
| 字段 | 含义 |
|------|------|
| `cluster_name` | 集群别名（后续 API 查询时传入的 `cluster_name`） |
| `cluster_alias` | 集群别名（同 `cluster_name`） |
| `cluster_display_name` | 集群中文名（用于展示） |
| `cluster_en_name` | 集群英文名（domain_prefix） |
| `gaia_id` | 集群 gaia_id（业务侧调度提交、申请权限等场景使用） |

直接将查询结果整理后返回给用户，不需进入下方的"队列资源分析"流程。

### 场景 B：队列资源分析（核心场景）

**触发**：用户要分析队列的资源使用、拥堵、资源大户、空跑任务、批量补录等。

通过调用 do_mcp API 服务获取两类数据：**队列级别的调度器快照**和**队列内各应用的资源采集数据**，从多个维度分析队列资源使用的合理性，并从队列使用者的角度给出可操作的优化建议。
...
2. **提供应用组名（不带集群名）**：先查询应用组有哪些集群（场景 A 入口），让用户选择后再分析
3. **提供 Application ID（app_id）**：先通过 app_id 查到应用组和集群，再进入分析流程
4. **只想看应用组有哪些可用集群 / 拿 gaia_id**：直接走场景 A，到 `queue-clusters` 即可结束，无需继续后续步骤
...
| 命令 | 功能 | 需要鉴权 |
|------|------|------|
| `do-bigdata yarn queue-clusters` | 查询应用组在哪些集群有资源，返回 cluster_name/alias/中文名/英文名/**gaia_id** | 是（CLI 内部处理） |
...
> [WARN] 性能约束：涉及 app 资源数据的命令（`queue-analysis`），**1 分钟内最多调用 2 次**。如需对比多个时间段，两次调用之间**等待至少 30 秒**。
...
**返回字段**（每个集群一条）：`cluster_name`（别名）、`cluster_alias`、`cluster_display_name`（中文名）、`cluster_en_name`（英文名）、`gaia_id`。

处理逻辑：
1. 如果用户提供了集群名 → 在返回的集群列表中匹配
2. 如果用户没有提供集群名 → 将集群列表展示给用户，请用户选择
3. 如果只有一个集群 → 直接使用
4. **如果用户只是想查询应用组可用集群或拿 gaia_id（场景 A）** → 把结果整理后返回给用户即可结束，无需继续后续步骤
