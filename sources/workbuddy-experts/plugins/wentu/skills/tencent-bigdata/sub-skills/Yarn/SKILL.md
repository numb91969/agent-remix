...
do-bigdata yarn log-list --container-url "<container_logs_url>" --query "<用户原始问题>"

# 快速查看日志尾部
do-bigdata yarn log-content --log-url {stderr_url} --start 8192 --query "<用户原始问题>"

# 分段读取日志内容（负偏移=从尾部）
do-bigdata yarn log-content --log-url "<log_url>" --start 8192 --query "<用户原始问题>"

# 搜索关键词（从尾部扫描 40MB 覆盖全量日志，支持正则）
do-bigdata yarn log-content --log-url {stderr_url} --start 40960000 --grep "ERROR" --query "<用户原始问题>"
do-bigdata yarn log-content --log-url {stderr_url} --start 40960000 --grep "Exception" --query "<用户原始问题>"
...
## 2. yarn-queue-analysis — YARN 队列资源分析 / 应用组可用集群查询

**适用场景**：
1. **队列资源分析**：用户需要分析 YARN 队列或应用组的资源使用情况，需提供队列名称。适用于判断队列是否拥堵、识别资源大户、发现空跑任务等场景。
2. **应用组可用计算集群查询**：用户只想查询某个应用组在哪些计算集群上有资源（含 `gaia_id`），不需要做完整的资源分析。

### 核心能力

| 能力 | 说明 |
|------|------|
| 应用组可用集群查询 | 根据应用组名返回可选集群列表，含 cluster_name/alias/中文名/英文名/**gaia_id** |
...
# 查询应用组在哪些集群有资源（返回含 gaia_id；轻量查询/选集群两用）
