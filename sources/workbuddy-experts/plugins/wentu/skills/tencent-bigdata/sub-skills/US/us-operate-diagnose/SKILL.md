...
> ### * 多地域（Namespace）规则
>
> **环境识别**：执行任何操作前，**必须按父 skill `US/SKILL.md` 的【Namespace 自动识别决策流】判定 namespace**（Step 1 URL 识别 U1~U6 → Step 2 关键词直判 → Step 3 仅提新加坡走【模板 T1】二次确认 → Step 4 default）。多 URL 冲突走【模板 T2】；URL 与口头声明冲突走【模板 T3】并**以 URL 为准**继续执行。
>
> **运行前声明**（统一措辞，与父 skill 一致）：
...
>
> **参数传递**：当 namespace 非 default 时，**所有** `do-bigdata us` 命令**必须**附加 `--skill-namespace <ns>` 参数，**包括 `execute-*` 命令**。
>
> **[WARN] overseas-sg 特殊限制**：`overseas-sg` 环境**只能使用 US 原生 API**，禁止调用 `describe-*` 类（`describe-log` / `describe-task-detail` / `describe-tasks` / `describe-execution-log` / `describe-execution-records` / `download-file`）—— 需要日志查询/任务详情时改用 US 原生命令（`log` / `stage-log` / `original-log` / `query-task` / `download-script`）。
>
...
> ```
>
> ### * 「参考已有任务创建」专项铁律（防止跨环境查询误判）
>
> 当用户说出 **"参考 / 基于 / 仿照 / 复制 任务 X 创建 Y"** 时，模型常见错误链路是：
>
> > [FAIL] **错误链路**：直接用 `describe-task-detail --task-id X` 查参考任务 → 漏传 `--skill-namespace` → 跑到默认国内 WeData 域名 → 任务返回不存在 / 字段不全 → 错误地告诉用户"任务可能在其他区域"。
>
> **正确链路（必须遵守）**：
>
> 1. **先确定参考任务所在环境**：
>    - 用户明确说明 → 直接采用
>    - 用户未说明 → **默认与"待创建任务"同环境**（用户说"创建新加坡任务"，参考任务也在 SG）
>    - 仍不确定 → 在响应中明确询问"参考任务所在环境（国内 / 新加坡）"，不要擅自猜测
>
> 2. **查询参考任务时优先级**：
>    - * **首选 `query-task`**（US 原生 API，跨环境支持最稳）：`do-bigdata us query-task --task-id <X> --skill-namespace <env> --query "查询参考任务"`
>    - * 备选 `describe-task-detail`（WeData OpenAPI 封装，task ID 17 位的纯 WeData 任务必走）：**同样必须带 `--skill-namespace <env>`**
>    - * 兜底：两者均失败再向用户报告
>
> 3. **跨环境拷贝（特例）**：用户明确说"参考国内的 X 建 SG 的 Y" → 查询用 `--skill-namespace default`，创建用 `--skill-namespace sg`，且响应中**显式说明两步分别用了哪个 namespace**。
>
> 4. **任务 ID 位数判定不能省**：18 位 → US 任务（用 `query-task`）；17 位 → 纯 WeData 任务（`query-task` 也支持，必要时再用 `describe-task-detail`）。
>
> *** 自检（响应给用户前必走）**：拼接的命令字符串中，**`--skill-namespace <env>` 是否真的出现了？** 如果 SG 环境下的命令里没有这个参数，立即修正后再执行。
...
>
> ### * 规则 1.1.1：公有云新加坡（overseas-sg）参数分流铁律（最高优先级）
>
> **触发条件**：当 `skill_namespace = overseas-sg` 且当前正在执行"创建任务"的 G1 参数收集 / 缺失参数展示环节时，本条铁律**强制生效**。
>
> **强制动作（必须做）**：
> 1. AI **必须**先打开并阅读**唯一数据源** `do_cli/sub-cli/US/common/overseas-sg-cluster-params.md`，按其中规则处理 `gaia_id` / `spark_version`。（原 `references/create-task-params.md` 中的 overseas-sg 章节已迁移为指针，只作跳转，不再维护清单内容。）
> 2. **SQL 类任务** — `taskType ∈ {66, 68, 75, 76, 102, 121, 132, 300, 301}`：
>    - `gaia_id` **直接预填为 `1386`**（公有云新加坡 SQL 类默认集群）
>    - 在 G1 缺失清单或参数确认表格中**明确告知**用户"gaia_id 已按公有云新加坡默认值 1386 使用，如需修改请告知"
>    - **禁止**将 gaia_id 列为需用户手填的缺失项
> 3. **Spark 类任务** — `taskType ∈ {128, 129, 208}`（硬性闭集）：
>    - `gaia_id` **必须**展示以下 4 集群清单让用户选择：`1=腾讯云新加坡wedata公共集群 / 4=新加坡天穹hok-offline集群 / 5=腾讯云新加坡midas-offline集群 / 6=腾讯云新加坡pubgm-offline集群`
>    - `spark_version` **必须**展示以下 2 个可选值让用户选择：`spark2.0-gaia3.2` / `spark3.3-gaia3.2`
>    - **禁止**让用户手填、**禁止**给内网 OA 链接
>
> **严格禁止（不得做）**：
> - [FAIL] **禁止**在 overseas-sg 环境下向用户展示 `tdwhelper.oa.com` / `tdwopen.oa.com` 等内网 OA 查询链接（公有云环境用户无法访问）
> - [FAIL] **禁止**在 overseas-sg 环境下要求用户手填 gaia_id / spark_version
> - [FAIL] **禁止**将本铁律的 SQL/Spark taskType 闭集与其他非闭集任务混淆处理
>
> **自检（G1 输出前必走）**：当前 skill_namespace 是否为 `overseas-sg`？如果是，`gaia_id`/`spark_version` 的处理是否已按上述规则？若未按，立即修正后再输出。
>
> ---
>
...
>
> **规则 4：重跑方式确认** — 执行重跑操作前，必须向用户展示以下 2 个重跑方式选项，让用户选择（默认 `11`）：
>
> | param | 含义 |
> |-------|------|
...
>
> [WARN] **OpenAPI不支持级联下游任务重跑**：
> - 原因：级联下游操作风险高，OpenAPI无法做充分筛选和校验
> - 解决方案：如需重跑下游，请移步WeData界面操作
>
> **展示前必须先获取链接**：在展示重跑选项前，先调用 `do-bigdata us get-wedata-url --task-id <id> --query "获取WeData链接"` 获取代码生成的 WeData 跳转链接（返回 JSON 中的 `wedata_url` 字段），然后直接在话术中展示该链接。
>
> **展示话术**（直接使用，禁止简化；`{wedata_url}` 用上面命令返回的实际链接替换）：
> ```
> 请选择重跑方式：
> - 11（默认）：仅当前任务，检查父任务状态
> - 12：仅当前任务，不检查父任务（直接调起）
>
> [WARN] 注意：OpenAPI不支持级联下游任务重跑。
> 原因：级联下游操作风险高，OpenAPI无法做充分筛选和校验。
> 解决方案：如需重跑下游，请移步WeData界面操作：
> * {wedata_url}
> ```
>
> **规则 4.1：回溯方式确认** — 执行回溯（backtrack）操作前，必须向用户展示以下 2 个回溯方式选项，让用户选择（默认 `11`）：
>
> | param | 含义 |
> |-------|------|
...
>
> [WARN] **OpenAPI不支持级联下游任务回溯**：
> - 原因：级联下游操作风险高，OpenAPI无法做充分筛选和校验
> - 解决方案：如需回溯下游，请移步WeData界面操作
>
> **展示前必须先获取链接**：在展示回溯选项前，先调用 `do-bigdata us get-wedata-url --task-id <id> --query "获取WeData链接"` 获取代码生成的 WeData 跳转链接（返回 JSON 中的 `wedata_url` 字段），然后直接在话术中展示该链接。
>
> **展示话术**（直接使用，禁止简化；`{wedata_url}` 用上面命令返回的实际链接替换）：
> ```
> 请选择回溯方式：
...
| **获取WeData链接** | `get-wedata-url` | `QueryTask` (GET) | 根据 task_id + namespace 生成 WeData 任务详情页跳转链接，**展示重跑/回溯选项时必须先调用** |
...
>
> 补录和回溯的**参数格式不同**（补录用 `check_parent` 布尔值，回溯用 `param` "11"/"12"），
> 因此 AI 必须在**向用户收集操作参数之前**先完成路由判断，才能按正确的参数格式引导用户：
>
> [WARN] **不再支持级联子任务补录/回溯**：OpenAPI不支持级联下游任务；原因：级联下游操作风险高，OpenAPI无法做充分筛选和校验；解决方案：如果要进行级联下游任务，请你移步WeData界面操作（代码自动生成 WeData 跳转链接）
>
> | 步骤 | 动作 | 说明 |
> |------|------|------|
> | **Step 0** | 调用 `do-bigdata us determine-route --task-id <id>` | 预判断走 register 还是 backtrack |
> | **Step 1** | 根据路由结果向用户收集参数 | register → 询问 `check_parent`（是否检查父任务）；backtrack → **必须展示 param 二选一选项**(11/12)，见规则 4.1 |
...
>
> [ALERT] **路由判断和 projectId 查询是 AI 内部自动完成的步骤，严禁向用户询问！**
>
> - [FAIL] **绝对禁止**问用户"走方式A还是方式B？" / "走 register 还是 backtrack？" / "走哪个接口？"
> - [FAIL] **绝对禁止**问用户"projectId / 项目ID 是多少？"（US 18 位任务时，projectId 从 `query-task` / `determine-route` 自动获取）
> - [OK] 正确做法：AI **静默**调用 `determine-route`（或 `query-task`），获取路由结果和 projectId 后，直接进入参数收集阶段
> - 仅当任务为 **17 位纯 WeData 任务**且 `determine-route` 返回需要用户提供 projectId 时，才可以向用户询问 projectId
>
> [WARN] **禁止反过来**：不要先按某种格式问用户参数，然后在 prepare 内部路由转发时强行转换参数——这会导致语义丢失（如回溯和补录的参数格式不同，无法互相表达）。
>
