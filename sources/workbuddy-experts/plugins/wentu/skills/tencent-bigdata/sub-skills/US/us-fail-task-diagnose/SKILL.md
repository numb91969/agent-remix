...
1. **环境识别** → **必须按父 skill `US/SKILL.md` 的【Namespace 自动识别决策流】判定 namespace**（Step 1 URL 识别 U1~U6 → Step 2 关键词直判 → Step 3 仅提新加坡走【模板 T1】二次确认 → Step 4 default）。多 URL 冲突走【模板 T2】；URL 与口头声明冲突走【模板 T3】并**以 URL 为准**继续执行。
   `overseas-sg` 环境**仅使用 US 原生 API**，`describe-*` 类命令禁用。识别完毕后输出环境声明（统一措辞）：
...
> ### *** 精确定位模式 — 级联诊断强制执行（禁止跳过）
>
> **[WARN] 精确定位模式同样必须执行级联诊断，不可因为"只分析一次执行"就跳过！**
>
> 采集完 stage-log / log / original-log 后，**禁止直接输出报告**，必须先：
>
> 1. **搜索关键 ID**：在所有已获取的日志中搜索 `application_\d+_\d+`（Application ID）和 UUID 格式的 Session/Connection ID
> 2. **加载级联规则**：执行 `do-bigdata docs show --skill us-fail-task-diagnose --file cascade-diagnosis.md`
> 3. **输出级联判断**：向用户输出「⏳ 级联诊断检查」进度（格式见上方"输出顺序约束"）
> 4. **执行级联**：有 Application ID → YARN 级联；有 Session/Connection ID → SuperSQL 级联；两者都有 → 同时执行
> 5. **合并输出**：级联完成后，当轮在同一条 final message 中输出「US 报告 + 级联结论 + 收尾」
>
> **禁止的错误模式**：
> - [FAIL] "精确定位模式只分析一次执行，不需要级联" → 错！级联是获取下游详细日志，与模式无关
> - [FAIL] "根因已经从 stage-log 中找到了，不需要级联" → 错！只要有关键 ID 就必须执行
> - [FAIL] 跳过级联直接进入"步骤 4 分析阶段" → 报告无效
...
> ### * 特殊场景专项诊断触发（强制 — 命中即按对应 SOP 输出，不可笼统回复）
>
> | 命中信号 | 必须执行的动作 | SOP 文档 |
> |---------|--------------|---------|
> | 任务**配置了自定义执行机器** + 实例「未下发 / 等待下发」（依赖与并发已排除） | **必须主动建议用户登录该执行机器执行 `ps aux \| grep loader` 检查 Runner/loader 进程是否存活**；未存活则给出 `cd /data/tscheduler/runner; ./start_loader.sh` 重启指引 + 排查文档 https://iwiki.woa.com/p/188168876 | `troubleshooting-guide.md` §3.1.1 / `error-patterns.md`「自定义执行机器任务未下发专项」 |
> | 出库 **ClickHouse** 报 `connect timed out` / `SocketTimeoutException: connect timed out` | **必须**告知"这类超时几乎全部是 CK 侧限制了网段（数平不限制访问）"，建议**从 Spark 计算节点 telnet 测 CK 连通性**，重点核对 CK 是否缺 **28/29 网段路由**，结论为需 CK 侧放通计算节点网段 | `troubleshooting-guide.md` §2.2 / `common-errors.md` §2.6 |
> | 任务**依赖关系异常**（依赖丢失/新增/与预期不符） | **必须**执行 `do-bigdata us change-log --task-id <ID>` 查询变更/审计记录，重点排查**人为 API/页面修改**导致的依赖异常（列出操作人 + 时间 + 变更前后依赖差异） | `troubleshooting-guide.md` §8 |
> | **平台运行账号 / 共享账号**负责人交接咨询 | 引导使用 **zhiyan 批量替换工具** http://zhiyan.oa.com/operate/352/task/#/task/result/858 + WeData 共享账号配置方式 | `troubleshooting-guide.md` §6.8 |
>
> 加载对应 SOP：`do-bigdata docs show --skill us-log-analyzer --file troubleshooting-guide.md`（CK/未下发/审计/交接）、`do-bigdata docs show --skill us-fail-task-diagnose --file error-patterns.md`（专项诊断流程）。
...
4. **结尾固定提示**：`[WARN] 如果US使用上有任何问题，可以直接联系 kimlinlin`
...
>
> 清理**必须且只能**通过 `do-bigdata us clean-cache` 子命令完成（**与本 SKILL 前文 Step 0 / 报告输出后清理段保持一致**），**严禁**使用以下任何方式：
>
