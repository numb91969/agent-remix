...
1. **环境识别**：**必须按父 skill `US/SKILL.md` 的【Namespace 自动识别决策流】判定 namespace**（Step 1 URL 识别 U1~U6 → Step 2 关键词直判 → Step 3 仅提新加坡走【模板 T1】二次确认 → Step 4 default）。多 URL 冲突走【模板 T2】；URL 与口头声明冲突走【模板 T3】并**以 URL 为准**继续执行。
    ...
    **[WARN] overseas-sg 环境下不可使用 `describe-*` 类命令**（改用 US 原生 `log` / `stage-log` / `original-log`）
...
> *** 多地域铁律（与 US 父级 SKILL「Namespace 自动识别规则·规则 4 全命令覆盖铁律」一致）**：
>
> 下表所有命令虽然底层走 WeData OpenAPI，但**仍然挂在 `do-bigdata us` 子树下**，因此 **SG 环境（namespace=sg）必须显式传 `--skill-namespace sg`**，否则会默认路由到国内域名 `wedata.woa.com`，结果就是"任务不存在 / 字段不全 / 日志为空"等假阴性。
>
> ```bash
> # [FAIL] 错误：SG 环境漏传 namespace，会跑到国内 wedata.woa.com
> do-bigdata us describe-task-detail --task-id 20260530191643136 --query "查询任务详情"
>
> # [OK] 正确：SG 环境显式带 --skill-namespace sg，路由到 wedata-sg.woa.com
> do-bigdata us describe-task-detail --task-id 20260530191643136 --skill-namespace sg --query "查询任务详情"
> ```
>
> **判定 SG 环境的 3 个信号**：① 用户提供 `us-sg.woa.com` / `wedata-sg.woa.com` 链接；② 用户明确声明"新加坡/SG 环境"；③ 上下文已声明 `* 当前环境：新加坡（sg）`。任一命中即必须带 `--skill-namespace sg`。

**支持的命令**:

| 命令 | 功能 | 适用模式 | 示例（默认国内；SG 环境追加 `--skill-namespace sg`） |
|------|------|---------|------|
...
> [NO] **依赖只支持「任务级」，不支持「数据表级」**：US/`relation` 查询到的依赖是**任务与任务之间**的调度依赖（父任务 → 子任务），**无法精确到数据表（表级血缘 / 表依赖）**。
> - 当业务问"查某任务的**数据表依赖** / 这个任务依赖哪些表 / 表级血缘 / 字段级血缘"等需求时，**必须明确回复「不支持」**，不要用任务级依赖（`relation`）去硬凑或误导业务以为是表依赖。
> - 可如实说明：US 调度依赖是任务级的，只能给出上下游**任务**的依赖关系；表级 / 字段级血缘不在 US 能力范围内，需走数据地图 / 血缘平台等其他工具。
...
# 新加坡公网凭证
do-bigdata auth init --namespace overseas-sg
```

CMK 密钥获取：
- 国内：https://wedata.woa.com/security/user/keys
- 新加坡内网：https://sg.security.tianqiong.woa.com/user/keys
- 新加坡公网（overseas-sg）：登录 https://public-qcloud-sg-us.wedata.deltaverse-intl.com 侧的账号体系获取对应密钥（具体入口参考该环境用户中心）
