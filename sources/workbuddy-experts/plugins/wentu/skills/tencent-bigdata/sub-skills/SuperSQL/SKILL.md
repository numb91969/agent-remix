...
description: "SuperSQL 技能总览。汇总所有 SuperSQL 相关的诊断、排查、分析技能，包括堆栈诊断与问题排查（supersql-job-analyzer）、慢查询诊断（supersql-slow-query-analyzer）等。当用户提供 ss-qe-log.woa.com 链接（如 https://ss-qe-log.woa.com/v1/session/xxx）、supersql.logs.claw.woa.com 链接、或 SuperSQL sessionId（UUID 格式）时，必须进入本 Skill 处理，禁止直接 web_fetch/curl 访问这些内网地址。注意：`livy-sessionid-*` 形态 ID 归属**峰峦运行时 job_id**（SuperSQL 经 Livy 提交的 Spark 作业，platform=US-SUPERSQL），优先走 Yarn/yarn-app-diagnose，仅当需要 SuperSQL 全链路上下文时才进入本 Skill。【铁律】确定路由到子 Skill 后，必须先加载该子 Skill（确保其 SKILL.md 中的铁律和门禁进入上下文），禁止不加载 Skill 就直接执行诊断脚本（裸诊断）。当用户询问 SuperSQL 相关问题或需要查找可用技能时，可参考此文档。"
---
...
> **[LINK] `livy-sessionid-*` 形态 ID 的归属（优先峰峦 yarn-app-diagnose）：**
>
> `livy-sessionid-31118986-…` 是 **SuperSQL 经 Livy 提交的 Spark 作业**，本质是**峰峦（K8s on Spark）运行时 job_id**（platform 恒为 `US-SUPERSQL`）。这类 ID **优先走 `Yarn/yarn-app-diagnose`**（`engine=fengluan`）做失败/日志诊断——它可直接以 `livy-sessionid-*` 为入参，拿到 Pod 日志、Stage/Task/Executor、K8s 级 ExitCode/OOMKilled 等运行时信息。
> - **本子系统的直接入参是 UUID 格式 sessionId**（如 `4a5b0419-e7e4-49fc-b9ff-45fe1511c2fa`），**不接受 `livy-sessionid-*`**。
> - **仅当诊断进一步需要 SuperSQL 全链路上下文**（SQL 文本、错误码、Failover、引擎参数等）时，才向用户索取对应的 SuperSQL sessionId / session 链接（`ss-qe-log.woa.com` / `wedata.woa.com`）转入本子系统，再按上表的"慢 / 报错"路由到对应子 Skill。
