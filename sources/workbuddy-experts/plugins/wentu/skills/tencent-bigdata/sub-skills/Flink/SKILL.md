...
| `sg_public_qcloud_oceanus2.0` | `public-qcloud-sg-oceanus.wedata.deltaverse-intl.com` | HTTPS 公网（无 IP 直连） | `https://public-qcloud-sg-oceanus.wedata.deltaverse-intl.com/#/task/streaming/detail/{project_id}/{job_id}/ops` |

> **重要**: 所有 Oceanus API 调用均默认使用 **IP 直连**（`http://{API_IP}:8080`），不通过域名访问，绕过 OA 网关和 SSL 证书问题。脚本内部会根据用户提供的域名 URL 自动映射到对应 IP。
>
> **海外公有云环境例外**：`sg_public_qcloud_oceanus2.0`（wedata sg_public）走 **HTTPS 公网**（不做 IP 直连、不做本机 DNS 解析）。使用时必须显式指定 `--skill-namespace overseas-sg` 以加载海外公有云凭证域（对应 CMK 从 https://public-qcloud-sg-sc.wedata.deltaverse-intl.com/user/keys 下载）。**注意与 `namespace=sg`（内网自研云 SG）区分**。
>
> **海外公有云 Flink UI 入口约定**：Oceanus execution 响应会带 `properties.yarnProxyUrl` 字段，值为 knox 网关代理的公网 HTTPS 地址（如 `https://public-qcloud-sg-knox.wedata.deltaverse-intl.com:8080/gateway/pubgm-qcloud-sg-inlong-yarn/yarn/cluster/app//application_xxxxx`）。CLI 侧 `http_helper.oceanus_get_execution` 会**自动把 `applicationUrl` 归一化为 `yarnProxyUrl`**（原始 URL 备份到 `_original_application_url`），下游所有 Flink UI REST 调用（`flink diag` / `flink-ui` / `checkpoint` / `metrics` 等）无需感知，直接读 `applicationUrl` 即可。MCP `api_forward` 白名单已放行 knox 域名 `public-qcloud-sg-knox.wedata.deltaverse-intl.com`。

**环境识别规则**：
1. **链接优先**：链接域名为 `public-qcloud-sg-oceanus.wedata.deltaverse-intl.com` → `namespace=overseas-sg`（海外公有云）
2. **用户声明**：用户明确声明"新加坡海外公有云/SG 公有云环境"（即使未提供 SG 域名链接） → `namespace=overseas-sg`
3. **内网 SG 声明**：用户声明"内网 SG/自研云 SG" → `namespace=sg`
4. **默认**：其他情况 → `namespace=default`
5. **冲突处理**：若链接域名与用户声明冲突（如声明海外公有云但提供国内域名链接），以链接为准并提示用户确认
6. **识别结果输出**：`* 当前环境：国内（default）` 或 `* 当前环境：新加坡海外公有云（overseas-sg）` 或 `* 当前环境：新加坡内网（sg）`
...
以下是 `do-bigdata flink` 实际注册的所有命令及其所属 Skill（共 32 个，所有命令带模块前缀，无冲突）：
...
| 7 | [Oceanus 监控指标查询](#7-oceanus-metrics-query--oceanus-监控指标查询) | `oceanus-metrics-query/` | Oceanus 监控指标查询，支持 StarRocks 历史指标（NGCP，走 MCP）、Flink UI 指标（多版本适配 1.7/1.9/1.13/1.15/2.1）；原 Hermes 平台侧指标已下线 |
...
**适用场景**：用户需要查询 Oceanus 平台上 Flink 作业的监控指标，包括 StarRocks 历史指标（NGCP API，走 MCP）、Flink UI 指标。支持 Flink 1.7/1.9/1.13/1.15/2.1 多版本适配。

> 注：原「平台侧指标（Hermes）」已随 Hermes 服务下线而移除；如需历史/回溯指标请统一使用 `starrocks`，运行时指标请使用 `flink-ui`。

**触发关键词**：指标、监控、TPS、延迟、Checkpoint、背压、吞吐量、metrics、connector 指标、operator 指标、Flink UI 指标、NGCP、StarRocks
...
```bash
# 查询 StarRocks 历史指标（NGCP，走 MCP）
do-bigdata flink starrocks --action query_job --project-id {project_id} --job-id {job_id} --env oc2 --query "<用户原始问题>"

# 一键 GC 分析
do-bigdata flink starrocks --action analyze_gc --project-id {project_id} --job-id {job_id} --env oc2 --query "<用户原始问题>"

# 查询 Flink UI 指标
do-bigdata flink flink-ui --action job_overview --project-id {project_id} --job-id {job_id} --query "<用户原始问题>"

# 从 Oceanus URL 自动提取
do-bigdata flink flink-ui --action job_overview --oceanus-url "https://oceanus.woa.com/#/task/streaming/detail/{project_id}/{job_id}/ops" --query "<用户原始问题>"
```
