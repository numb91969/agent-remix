...
| `sg_public_qcloud_oceanus2.0` | `public-qcloud-sg-oceanus.wedata.deltaverse-intl.com` | `https://public-qcloud-sg-oceanus.wedata.deltaverse-intl.com/#/task/streaming/detail/{project_id}/{job_id}/ops` |

> **海外公有云环境**：`sg_public_qcloud_oceanus2.0` 走 HTTPS 公网（不做 IP 直连），使用时须附加 `--skill-namespace overseas-sg` 加载海外公有云凭证（对应 CMK 从 https://public-qcloud-sg-sc.wedata.deltaverse-intl.com/user/keys 下载）。
