...
内置事实性资料（均登记于 `references/design-sources.md`，标注来源、适用范围和风险等级）：

- `references/regulations-quick-reference.md`：《大型群众性活动安全管理条例》（国务院令第505号）与《高校学生社团建设管理办法》（教党〔2020〕13号）关键条款速查，含许可门槛、法定时限、申请材料和罚则。
- `references/campus-approval-common-pattern.md`：多校公开规定提炼的审批共性模式（规模分档、时间窗、材料组合、审批链、宣传与经费纪律）与用户核实清单。
- `references/campus-safety-and-privacy.md`：安全与隐私边界，含参加者构成检查和合同、财务、声誉风险清单。
- `assets/emergency-plan-skeleton.md`：应急预案骨架（组织指挥、工作组分工、处置流程、信息报告），大型活动按需调用。
