...
> **本文件搬迁自 inquiry-price-parallel/references/input-parsing.md（2026-06-26）**：
> 内容完全一致 · parallel 处历史保留供 backward-compat · 新代码请引用本文件路径。
> 本文件出现的 `build_proposal.py` / `split_tasks.py` 引用是 parallel core 的内部脚本 ·
> 与 wrapper 的 `parse_input.py` 协同：wrapper 用 parse_input 转 markdown ·
> parallel core 内部再用 build_proposal / split_tasks 把 markdown fan-out 成 tasks。
