...
> **与 westock-data 的分工**（本包只有这两个子工具；源包里的 `westock-portfolio` 未收录）：
> - **westock-tool**（本工具）：**筛选/选股** — 条件选股 / 策略选股 / 标签选股
> - **westock-data**：**查询个股详情** — "查某只股票的行情/K线/财务/资金等数据"；概念股/板块成份请用 `westock-data sector`
...
**运行方式**：本 skill 与 westock-data 合并打包，两者各有自己的入口脚本，
本工具的入口是 **`scripts/tool-index.js`**（数据查询工具是 `data-index.js`）：

```bash
node <skill-path>/scripts/tool-index.js filter "ClosePrice >= 100"
```

> ⚠️ **下文示例里的 `westock-tool <命令>` 是简写，一律等价于
> `node <skill-path>/scripts/tool-index.js <命令>`**。这个短命令**没有随包安装到 PATH**
> （包内没有 `bin/` 目录），照抄短命令会报 command not found。
> 即便机器上恰好有同名可执行文件，那也可能是一份与本包无关的旧版拷贝，**优先走上面的完整路径**。
...
**完整预设函数列表（含默认参数）参见 [references/ai_usage_guide.md](./references/tool-ai-guide.md)**
...
**完整策略列表参见 [references/ai_usage_guide.md](./references/tool-ai-guide.md)**
...
**完整标签列表参见 [references/ai_usage_guide.md](./references/tool-ai-guide.md)**
...
**完整字段速查表（含行情、财务、技术指标等全部字段）参见 [references/fields-guide.md](./references/tool-fields-guide.md)**

**详细返回格式、分析模板参见 [references/tool-ai-guide.md](./references/tool-ai-guide.md)**
...
**更多场景示例参见 [references/scenarios-guide.md](./references/tool-scenarios-guide.md)**
...
见文首「环境要求 / 运行方式」。入口脚本：`scripts/tool-index.js`。
