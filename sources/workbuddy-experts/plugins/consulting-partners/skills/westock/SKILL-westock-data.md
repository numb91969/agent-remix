...
**运行方式**：本 skill 与 westock-tool 合并打包，两者各有自己的入口脚本，
本工具的入口是 **`scripts/data-index.js`**（选股工具是 `tool-index.js`）：

```bash
node <skill-path>/scripts/data-index.js search 茅台
```

> ⚠️ **下文示例里的 `westock-data <命令>` 是简写，一律等价于
> `node <skill-path>/scripts/data-index.js <命令>`**。这个短命令**没有随包安装到 PATH**
> （包内没有 `bin/` 目录），照抄短命令会报 command not found。
> 即便机器上恰好有同名可执行文件，那也可能是一份与本包无关的旧版拷贝，**优先走上面的完整路径**。


> **与 westock-tool 的分工**（本包只有这两个子工具；源包里的 `westock-portfolio` 未收录）：
> - **westock-data**（本工具）：查询个股详情 — "查某只股票的行情/K线/财务/资金/宏观/板块成份等数据"
> - **westock-tool**：筛选/选股 — "找出满足条件的股票列表"（条件选股 / 策略选股 / 标签选股）
...
> `search` 和 `minute` 不支持批量查询。详细返回格式见 [references/ai_usage_guide.md](./references/data-ai-guide.md)
...
> 以上命令均支持逗号分隔批量查询。详细字段说明见 [references/ai_usage_guide.md](./references/data-ai-guide.md)
...
**完整分析场景（33个）参见 [references/scenarios-guide.md](./references/data-scenarios-guide.md)**

**详细返回格式、字段说明、分析模板参见 [references/ai_usage_guide.md](./references/data-ai-guide.md)**
...
见文首「环境要求 / 运行方式」。入口脚本：`scripts/data-index.js`。
