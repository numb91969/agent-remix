...
- 已取得的时间序列、行业资金流排名、方向对比或事件窗口数据用图形能明显降低理解成本。
...
2. Identify the smallest useful visual: candlestick/volume, line trend, signed industry-fund-flow bars, event-return bars, grouped comparison, line-column combination, calibrated radar or eligible report image.
...
- For `industry_fund_flow_ranking`, normalize the returned ranking into the signed comparison family; for `industry_fund_flow_series`, normalize one or two industries into the trend family. Keep the returned dates, values, units and gaps unchanged. 按中国市场习惯用红色表示正值/净流入、绿色表示负值/净流出，并保留清晰零轴。
