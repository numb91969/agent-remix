...
0. 调用任何业务工具前，必须先读取对应 Layer 1 契约；不要只凭工具名试参数。行业多空风向标至少读取：
   ...
   - `layer1-fin-data/SKILL.md` 及 `references/entity.md`、`references/market.md`、`references/fund_flow.md`、`references/macro_financial.md`：篮子/指数、行情序列、申万一级行业资金流、估值
   ...
   - Shenwan fund flow：用户问板块资金、资金确认、资金流趋势时，优先复用 resolver 返回的标准申万一级代码；已知标准代码时调用 `get_industry_fund_flow_series({"industry_codes":["<returned sw level-1 code>"],"limit":20})`。用户问全市场板块资金榜时直接调用 `rank_industry_fund_flows({"rank_window":1})`，不需要为 31 个行业逐个解析。没有申万一级映射时跳过该证据，不要用同名概念、篮子或产业链节点替代。
...
5.1. 如果用户问资金流、资金确认或行业资金趋势，调用 `rank_industry_fund_flows` 或批量 `get_industry_fund_flow_series`。必须展示实际 `trade_date`、`coverage` 和 1/5/20 日样本完整性；日频资金流不得写成盘中实时，也不得把“主力净流入”改写成机构或北向资金。
...
- 行业资金流只作为市场行为证据。正净流入不等于价格必涨，负净流入不等于价格必跌；图表使用中国市场红正绿负，并保留零轴。
