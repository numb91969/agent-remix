...
| 端点 | 方法 | 用途 | 响应关键字段 |
|------|------|------|------------|
| `<local-user-path> | POST | 线索统计 | `{thisYearClue, thisMonthClue, thisWeekClue, todayClue}` 各含 `count` + 上期 |
| `<local-user-path> | POST | 商机统计 | 同上 + `amount` |
| `<local-user-path> | POST | 赢单统计 | 同上 |
| `<local-user-path> | POST | 进行中商机 | 同上 |
