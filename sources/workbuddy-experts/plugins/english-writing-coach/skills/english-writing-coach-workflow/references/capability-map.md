...
本专家包独立提供诊断与训练能力，不需要安装、调用或推荐包外市场 Skill；学习档案、看板、文档、剪藏和提醒按需联动平台原生能力：
...
15. 经用户授权，在其资料库保存英语错误、训练记录和语境生词；后续按历史薄弱项和到期记录继续训练。
16. 复习日期的确定性计算，以及按需创建一条批次提醒。
17. 在记录足够且用户明确要求时生成关联三张档案表的动态英语学习看板；按需保存精读句卡文档或剪藏材料链接。
...
- `references/library-integration.md`：资料库定位、三表 schema、授权、查询路由、复习状态机和降级规则。
- `references/dashboard-schema.md`：动态看板配置、数据口径、上线流程和增量更新纪律。
- `references/design-sources.md`：训练与联动设计方法的公开来源、适用范围和使用边界。
...
- `scripts/render_report.py`：严格校验考试、来源、整数评分、档次和词数一致性的作文批改 HTML 报告渲染。
- `scripts/review_schedule.py`：复习状态、阶段和日期的确定性计算。
- `scripts/render_dashboard.py`：把三张真实 database ID 注入动态学习看板母本，不固化档案数据快照。
- `templates/learning-dashboard.html`：分页读取三张表的动态资料库 Page 母本。
- `tests/`：脚本与 v1.4.1 场景规则回归测试。
...
- 未授权或平台原生能力不可用时只提供会话内学习卡；授权后持久化、提醒与看板以实际平台结果为准。
- 3000 词以上文本分段处理。
