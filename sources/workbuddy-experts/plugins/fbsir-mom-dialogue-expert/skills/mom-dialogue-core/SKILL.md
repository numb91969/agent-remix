...
已有项目先按 `references/upgrade-and-media-entry.md` 执行只读 `mom.py inspect/resume`，恢复原作品和下一步。版本升级本身不初始化、迁移、重扫或重分析。新增媒体按需启用独立扩展；原有来源、权限、人工确认和作品继续有效。

用户已提供音频或视频的时间码转写时，先完成来源提交和技术时长解析，再导入项目 `14_媒体处理/inbox/` 中的转写文件，创建私密候选故事。它只使用提供的原话和时间码，不产生模型确认。家人确认必须走同一参考中的哈希绑定确认步骤；确认、入稿和公开分别处理。
