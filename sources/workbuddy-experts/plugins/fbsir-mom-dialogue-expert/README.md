...
版本 26.9.6。旧项目先从原作品与下一步续接，核心格式继续为 2.0；批量媒体能力通过独立 `14_媒体处理/` 扩展按需启用，不要求重新建立资料库。

统一命令入口为 `skills/mom-dialogue-core/scripts/mom.py`，使用方法见包内 `references/upgrade-and-media-entry.md`。媒体技术解析与访问副本使用本机已有、经版本和哈希核对的 FFmpeg/FFprobe；本包不携带或自动下载这些程序。缺失时保留待处理项，既有项目和作品继续可用。技术解析、音轨抽取与图片/视频理解是不同状态。

新增的候选故事默认私密、待确认、不可入稿。家人以哈希绑定确认文件核对候选后，故事可成为“私密已确认”，原候选会保留为版本前像；这仍不授予入稿或公开权。交付始终通过原有权限选择和权限校验门。

入稿前，故事与每个使用来源都必须有单独的、显式记录的入稿许可；通过后故事仍默认私密。公开范围需要单独许可和原有公开权限门，不能由确认或入稿授权推导。
