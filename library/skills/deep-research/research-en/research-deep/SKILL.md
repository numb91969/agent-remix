...
description: Developed by Microsoft/GitHub, first mainstream AI coding assistant, ~40% market share, output structured JSON to <local-user-path>

## Field Definitions
Read <local-user-path> to get all field definitions
...
## Validation
After completing JSON output, run validation script to ensure complete field coverage:
python ~/.claude/skills/research/validate_json.py -f <local-user-path> -j <local-user-path>
Task is complete only after validation passes.
```
