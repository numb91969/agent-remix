...
> 格式规范见 `.agent/skills/cloud-mapping-import/references/variants/intl/format-product-strategy.md`。
...
AWS CLB (SLB→CLB) 推荐: direct_mapping 匹配: {"elb_type":"ALB/NLB/GWLB类型映射"} 备注: AWS ELBv2类型:application/network/gateway
AWS COS (OSS→COS) 推荐: direct_mapping 匹配: {"storage_type":"1:1精确映射","size":"保持原值"} 备注: AWS S3存储类型已在枚举映射表中
AWS CFS (NAS→CFS) 推荐: engine_grpc 匹配: {"performance_mode":"性能模式映射","throughput_mode":"吞吐模式映射"} 备注: AWS EFS性能模式:generalPurpose/maxIO
AWS VPN (VPN→VPN) 推荐: direct_mapping 匹配: {"vpn_type":"SiteToSite/ClientEndpoint类型映射"} 备注: AWS VPN分Site-to-Site和Client VPN
...
AWS TCR (ECR→TCR) 推荐: engine_grpc 匹配: {"service":"产品级映射"} 备注: Amazon EC2 Container Registry (ECR)→腾讯云TCR
AWS TCR (ECR Public→TCR) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Amazon Elastic Container Registry Public→腾讯云TCR
AWS DNSPod (Route53→DNSPod) 推荐: free 匹配: {"service":"产品级映射"} 备注: Amazon Route 53→腾讯云DNSPod
...
AWS CKafka (MSK→CKafka) 推荐: engine_grpc 匹配: {"service":"产品级映射"} 备注: Amazon Managed Streaming for Apache Kafka→腾讯云CKafka
AWS DLC (Athena→DLC) 推荐: engine_grpc 匹配: {"service":"产品级映射"} 备注: Amazon Athena→腾讯云DLC
AWS DTS (DMS→DTS) 推荐: engine_grpc 匹配: {"service":"产品级映射"} 备注: AWS Database Migration Service→腾讯云DTS
...
GCP CLB (SLB→CLB) 推荐: direct_mapping 匹配: {"lb_type":"类型映射"}
GCP COS (OSS→COS) 推荐: direct_mapping 匹配: {"storage_type":"1:1精确映射","size":"保持原值"} 备注: GCP存储类型已在枚举映射表中
...
GCP NAT (NAT→NAT) 推荐: direct_mapping 匹配: {"spec":"规格映射"}
GCP VPN (VPN→VPN) 推荐: direct_mapping 匹配: {"bandwidth":">=最近有效值"}
GCP Redis (Redis→Redis) 推荐: engine_grpc 匹配: {"capacity":">=最近有效值","version":">=最近版本"}
GCP MySQL (RDS→CDB) 推荐: direct_mapping 匹配: {"cpu":">=最近","memory":">=最近","disk_size":">=最近","version":"直传"} 排序: CPU升序→Memory升序
GCP PostgreSQL (RDS→PostgreSQL) 推荐: direct_mapping 匹配: {"cpu":">=最近","memory":">=最近","disk_size":">=最近","version":"直传"} 排序: CPU升序→Memory升序
...
GCP DNSPod (CloudDNS→DNSPod) 推荐: free 匹配: {"service":"产品级映射"} 备注: Google Cloud DNS→腾讯云DNSPod
...
AWS NAT (AmazonEC2:NatGateway→NAT) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Amazon VPC NAT Gateway→腾讯云NAT
Azure CBS (Storage:Disk→CBS) 推荐: engine_grpc 匹配: {"disk_type":"查表映射","disk_size":"边界限幅"} 备注: Azure托管磁盘→腾讯云CBS
Azure CDB (Database for MySQL→CDB) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Database for MySQL→腾讯云CDB
Azure CFW (Azure Firewall→CFW) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Firewall→腾讯云CFW
Azure CLB (Application Gateway/Load Balancer→CLB) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Application Gateway / Load Balancer→腾讯云CLB
Azure COS (Storage:Blob→COS) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Blob Storage→腾讯云COS
Azure CVM (VirtualMachines→CVM) 推荐: engine_grpc 匹配: {"cpu":">=向上取整","memory":">=向上取整","instance_family":"查表映射(1:N候选族列表)"} 排序: CPU升序→Memory升序，取最小满足规格 兜底: 三级兜底:1)指定族匹配 2)扩大族范围 3)全量规格 备注: Azure VM实例族映射仅Standard策略
Azure DNSPod (Azure DNS→DNSPod) 推荐: free 匹配: {"service":"产品级映射"} 备注: Azure DNS→腾讯云DNSPod（免费）
Azure KMS (Key Vault→KMS) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Key Vault→腾讯云KMS
Azure NAT (NAT Gateway→NAT) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure NAT Gateway→腾讯云NAT
Azure PostgreSQL (Database for PostgreSQL→PostgreSQL) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Database for PostgreSQL→腾讯云PostgreSQL
Azure TCR (Container Registry→TCR) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Container Registry→腾讯云TCR
Azure TKE (Azure Kubernetes Service→TKE) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure Kubernetes Service→腾讯云TKE
Azure VPN (VPN Gateway→VPN) 推荐: direct_mapping 匹配: {"service":"产品级映射"} 备注: Azure VPN Gateway→腾讯云VPN
