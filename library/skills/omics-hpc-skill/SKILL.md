...
> 1. 用 `run_hpc_command.py --command 'cat > <local-user-path> <<EOF\n...\nEOF\nsbatch <local-user-path>
...
Step 3：下发时加 --run-as-user
  python3 scripts/run_hpc_command.py \
    ...
    --command 'sbatch <local-user-path><user>/job.sh' \
    --run-as-user <username>

Step 4：脚本内部自动包装为：
  su - <username> -c 'sbatch <local-user-path><user>/job.sh'
```
...
python3 scripts/run_hpc_command.py \
  ...
  --command 'cat > <local-user-path> << '\''EOF'\''
#!/bin/bash
...
python3 scripts/run_hpc_command.py \
  --cluster-id hpc-9jragud9 \
  --command 'sbatch <local-user-path> \
  --run-as-user alice
```
...
python3 scripts/run_hpc_command.py \
  --cluster-id hpc-9jragud9 \
  --command 'qsub -N sge_job -l gpu=1 <local-user-path> \
  --run-as-user bob
```
