...
description: HR 预置指标查询工具型 Skill，由 hr-data-router 内部调用。适用于离职率、流入流出率、人员结构占比、平均年龄/工龄/司龄及上级组织对比等已由指标 API 封装口径的通用 HR 指标；社招简历、面试、Offer、入职和需求等招聘过程 KPI 仍优先使用 recruit-data-dashboard。指标无匹配或无权限时降级到 hr-data-sql-builder。
...
```bash
bash ../../scripts/track_skill_event.sh "0WEB06ZI7OVDOZQW" "indicator-query" "skill_invoked"
```

- 每次进入都执行一次，失败不阻塞主流程；
- 不上传用户原话、组织名称、候选人信息或查询结果；
- 执行后再进入正文流程。

## 入口边界与按需规则

- 本 Skill 默认不接受一级 Agent 的语义直达，由 `hr-data-router · I` 调用；
- 社招治理级招聘过程指标优先 `recruit-data-dashboard`，不要被“率/占比”字样截流；
- 处理返回结果时读取 [`hr-data-desensitization.md`](../../agents/references/data-rules/hr-data-desensitization.md)；
- MCP 依赖为 `hr_data_service_v1`，所需资源为 `data-view://indicators`、`data-view://dimensions`。
...
─── ① 该参数与用户问题有关吗？ ───
    ...
    a) org（组织）参数：必填，从 `data-view://dimensions` 实时匹配 `dimension_code == parameter_code`
        （或维度描述明确为组织的当前记录），使用其 `sql_script` 解析组织值
        → 跳过本步其余判断及①.5，直接进入②按「取值优先级」取值；禁止硬编码维度记录 ID
...
─── ② 取值 ───
    ...
    ELSE (string/date/int/Float):
        ...
        取值优先级（命中即停，不再往下走）：
        1. **会话级缓存**：本轮对话中此前已解析过的"组织名→orgId8"映射，直接复用；候选簇重试时也复用。
        2. **动态维度查询**：读取 `data-view://dimensions` 中 org 对应的 `sql_script`，调用 `starrocks_query` 解析当前组织 ID。多个组织名必须合并成一次 SQL 批量取值，禁止逐个查询；查得结果写入会话级缓存。

        禁止在 Skill 内长期硬编码组织 ID。组织架构和编码可能变化，始终以 MCP 维度资源的实时结果为准。
...
- 组织 ID 只允许来自 `data-view://dimensions` 对应的实时维度查询；
- 同一会话可缓存“组织名 → orgId8”，会话结束即失效；
- 候选指标重试复用会话缓存，不重复查库；
- 组织无法唯一匹配时只问一次澄清，不猜测、不使用历史硬编码值。
