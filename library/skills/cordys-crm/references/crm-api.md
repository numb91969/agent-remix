...
| 端点 | 用途 | 请求体 |
|------|------|--------|
| `POST <local-user-path> | 线索统计 | `HomeStatisticBaseSearchRequest` |
| `POST <local-user-path> | 商机统计 | 同上 |
| `POST <local-user-path> | 赢单统计 | 同上 |
| `POST <local-user-path> | 进行中商机统计 | 同上 |
| `GET <local-user-path> | 用户部门权限树 | — |
