...
```
执行：
  1. POST <local-user-path> (searchType=ALL) → 全公司线索数
  2. POST <local-user-path> (searchType=ALL) → 全公司商机+金额
  3. POST <local-user-path> (searchType=ALL) → 赢单
...
```
执行：
  1. GET <local-user-path> → 一级部门列表
  2. 每部门 POST <local-user-path> (searchType=DEPARTMENT)
