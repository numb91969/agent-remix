...
4. Run `layer2-industry-brief` to collect market, valuation and eligible申万一级日频资金流 evidence, news and reports, graph factors and anomalies, Same Boat viewpoints, and source-review URLs within an explicit evidence window. Only query fund-flow history when the resolver returned a stable SW level-1 code; otherwise keep that evidence family absent rather than substituting a concept or related industry.
...
- Daily industry fund flow must retain its returned trade date and supplier-defined meaning. It cannot be presented as realtime, institutional, northbound or account-level flow.
